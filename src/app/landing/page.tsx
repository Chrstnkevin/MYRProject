      ...
      <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px", cursor: "pointer" }} onClick={() => scrollTo("hero")}>
          <div style={{
            ...
            fontSize: "18px", boxShadow: `0 4px 12px ${C.primary}40`,
          }}>🍃</div>
          <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "4px" }} className="desktop-nav">
          ...
          <button onClick={() => window.location.href="/landing/supporting"}
            ...
            onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.boxShadow = `0 8px 20px ${C.primary}50` }}
      ...
      {menuOpen && (
        <div style={{
          ...
          {[
            ...
            <a key={n.label} href={n.href}
              style={{ display: "block", width: "100%", textAlign: "left", padding: "12px 0", fontSize: "16px", fontWeight: 500, color: C.dark, borderBottom: `1px solid ${C.border}`, fontFamily: "'DM Sans',sans-serif", textDecoration: "none" }}>
              {n.label}
            </a>
          ))}
          <button onClick={() => scrollTo("supporting")}
            style={{ marginTop: "16px", width: "100%", background: C.primary, border: "none", cursor: "pointer", padding: "14px", borderRadius: "12px", fontSize: "15px", fontWeight: 700, color: "white", fontFamily: "'DM Sans',sans-serif" }}>
            Get Started →
          </button>
        </div>
      )}
...
function Hero() {
  ...
  return (
    <section id="hero" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "100px clamp(20px,5vw,80px) 60px", position: "relative", overflow: "hidden", background: `radial-gradient(ellipse 80% 50% at 50% 0%, ${C.primary}12 0%, transparent 65%), #FAFAF8` }}>
      {/* Grid */}
      ...
      {mounted && (
        <div className="floating-cards">
          {[
            { emoji: "📊", label: "SFA Dashboard", sub: "98.2% uptime", color: "#E8F5E9", accent: "#43A047", x: "4%", y: "25%", delay: "0s" },
            { emoji: "💰", label: "Ficom Module", sub: "Live tracking", color: "#FFF3E0", accent: "#FB8C00", x: "78%", y: "20%", delay: "1s" },
            { emoji: "🎯", label: "Target Hit", sub: "↑ 24% this month", color: "#FCE4EC", accent: "#E91E63", x: "80%", y: "65%", delay: "0.5s" },
          ].map((card, i) => (
            <div key={i} style={{ position: "absolute", left: card.x, top: card.y, background: "white", borderRadius: "16px", padding: "12px 16px", boxShadow: "0 8px 32px rgba(0,0,0,0.09)", border: `1.5px solid ${card.color}`, animation: `float 6s ease-in-out ${card.delay} infinite`, minWidth: "150px", zIndex: 2, display: "flex", alignItems: "center", gap: "10px" }}>
      ...
      {/* Main content */}
      <div style={{ textAlign: "center", maxWidth: "720px", position: "relative", zIndex: 3, animation: mounted ? "fadeUp 0.7s ease both" : "none" }}>
        ...
        <div style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap" }}>
          <button onClick={() => document.getElementById("applications")?.scrollIntoView({ behavior: "smooth" })}
            style={{ background: C.primary, border: "none", cursor: "pointer", padding: "clamp(12px,2vw,16px) clamp(24px,4vw,36px)", borderRadius: "99px", fontSize: "clamp(13px,1.5vw,16px)", fontWeight: 700, color: "white", boxShadow: `0 8px 24px ${C.primary}40`, transition: "all 0.2s", fontFamily: "'DM Sans',sans-serif" }}
          ...
          <button onClick={() => document.getElementById("supporting")?.scrollIntoView({ behavior: "smooth" })}
            style={{ background: "white", border: `1.5px solid ${C.border}`, cursor: "pointer", padding: "clamp(12px,2vw,16px) clamp(24px,4vw,36px)", borderRadius: "99px", fontSize: "clamp(13px,1.5vw,16px)", fontWeight: 500, color: C.dark, transition: "all 0.2s", fontFamily: "'DM Sans',sans-serif", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "clamp(32px,5vw,80px)", alignItems: "center" }}>
        <div style={{ opacity: inView ? 1 : 0, transform: inView ? "none" : "translateX(-30px)", transition: "all 0.7s ease" }}>
          ...
          <h2 style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: "clamp(28px,4vw,48px)", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-0.02em", marginBottom: "20px" }}>
            Built for the{" "}
            <span style={{ color: C.primary }}>Philippines</span>{" "}
            Support Team
          </h2>
          ...
          <p style={{ fontSize: "clamp(14px,1.5vw,16px)", color: C.gray, lineHeight: 1.8 }}>
            From sales force tracking with <strong style={{ color: C.dark }}>SFA</strong> to financial management with <strong style={{ color: C.dark }}>Ficom</strong> and mobile operations with <strong style={{ color: C.dark }}>Ficom Lite</strong> — everything your team needs is right here.
          </p>
        </div>
...
function Applications() {
  ...
  const apps = [
    ...
    { emoji: "💰", name: "Ficom", full: "Finance & Commerce", color: "#FB8C00", bg: "#FFF8F0", desc: "Complete financial transaction management, outlet coverage tracking, and automated period-closing reports for accurate accounting.", tags: ["Transaction Mgmt", "Period Closing", "Outlet Coverage"], stat: { num: "99%", label: "Report Accuracy" } },
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: "clamp(40px,6vw,64px)", opacity: inView ? 1 : 0, transform: inView ? "none" : "translateY(20px)", transition: "all 0.6s ease" }}>
          ...
          <h2 style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: "clamp(28px,4vw,48px)", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-0.02em" }}>
            ...
            <span style={{ color: C.primary }}>One Unified Hub</span>
          </h2>
        </div>
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", textAlign: "center" }}>
        <div style={{ marginBottom: "clamp(40px,6vw,64px)", opacity: inView ? 1 : 0, transition: "all 0.6s" }}>
          ...
          <h2 style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: "clamp(28px,4vw,48px)", fontWeight: 800, color: C.white, lineHeight: 1.1, letterSpacing: "-0.02em" }}>
            Impact That Speaks<br/>
            <span style={{ color: C.primaryLight }}>for Itself</span>
          </h2>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "clamp(16px,2vw,24px)" }}>
          {stats.map((s, i) => (
            <div key={i} style={{ background: "rgba(255,255,255,0.06)", borderRadius: "24px", padding: "clamp(24px,3vw,36px) clamp(16px,2vw,24px)", border: "1px solid rgba(255,255,255,0.08)", backdropFilter: "blur(10px)", transition: "all 0.25s", opacity: inView ? 1 : 0, transform: inView ? "none" : "translateY(24px)", transitionDelay: `${i * 0.1}s` }}
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: "clamp(40px,6vw,64px)", opacity: inView ? 1 : 0, transition: "all 0.6s" }}>
          ...
          <h2 style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: "clamp(28px,4vw,48px)", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-0.02em", marginBottom: "14px" }}>
            Learn, Reference,<br/>
            <span style={{ color: C.primary }}>and Grow</span>
          </h2>
          <p style={{ fontSize: "clamp(14px,1.5vw,16px)", color: C.gray, maxWidth: "480px", margin: "0 auto", lineHeight: 1.7 }}>
